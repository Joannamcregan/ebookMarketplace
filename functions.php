...
function assignCreatorMemberRole($form_id, $response) {
    if( $response['success']  && ($form_id ==4212 /* Creator-Member Signup */ || $form_id ==6297 /* creator-membership for organizations*/)){
        ...
        $insertQuery = 'insert into %i (user_id, meta_key, meta_value) values (%d, "_vendor_commission", "100");';
...
function mp_modify_unsubscribe_confirmation_page( $HTML, $unsubscribeUrl ) {
	$HTML = '<hr>';
 	$HTML .= '<center>You can <a href="'.$unsubscribeUrl.'">click here</a> to unsubscribe from our newsletter.</center>';
 	$HTML .= '<hr>';
    return $HTML;
}